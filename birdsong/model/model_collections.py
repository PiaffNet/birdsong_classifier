...
def downsampling(x, filters):
  ### Branche 1
  b1= layers.Conv2D(filters, kernel_size = 1, activation = 'relu') (x)
  b1 = layers.Conv2D(filters, kernel_size = 3, strides = 2, padding = 'same',activation = 'relu') (b1)
  b1= layers.Conv2D(filters, kernel_size = 1, activation = 'relu') (b1)

  ### Branche 2
  b2 = layers.AveragePooling2D(pool_size = (2,2), strides = 2, padding = 'same') (x)
  b2= layers.Conv2D(filters, kernel_size = 1, activation = 'relu') (b2)

  out = layers.Add()([b1,b2])
  out = layers.ReLU()(out)
  return out
...
def build_archi_2(num_classes: int, input_shape=(64,376,1)) -> Model:
        #preprocessing
    visible = Input(shape=input_shape)
    conv1 = layers.Conv2D(32, kernel_size=5, padding ='same', activation='relu')(visible)
    bn1 = layers.BatchNormalization() (conv1)
    relu1 = layers.ReLU() (bn1)
    pool1 = layers.MaxPooling2D(pool_size=(1, 2))(relu1)
    ...
    conv5= layers.Conv2D(64, kernel_size = 3, strides = 4, padding = 'same', activation = 'relu') (res1)
    bn5 = layers.BatchNormalization() (conv5)
    relu5 = layers.ReLU() (bn5)
    drop5 = layers.Dropout(rate=0.2) (relu5)

    conv6= layers.Conv2D(32, kernel_size = 3, padding = 'same', activation = 'relu') (drop5)
    bn6 = layers.BatchNormalization() (conv6)
    relu6 = layers.ReLU() (bn6)
    drop6 = layers.Dropout(rate=0.26) (relu6)

    conv7 = layers.Conv2D(16, kernel_size = 3, padding = 'same', activation = 'relu') (drop6)
...
"""
def build_archi_3(num_classes: int, input_shape=(64,376,1)) -> Model:
    #preprocessing
    visible = Input(shape=input_shape)
    conv1 = layers.Conv2D(32, kernel_size=5, padding ='same', activation='relu')(visible)
    bn1 = layers.BatchNormalization() (conv1)
    ...
    # Classification
    conv5= layers.Conv2D(512, kernel_size = 3, strides = 4, padding = 'same', activation = 'relu') (res4)
    bn5 = layers.BatchNormalization() (conv5)
    relu5 = layers.ReLU() (bn5)
    drop5 = layers.Dropout(rate=0.24) (relu5)

    conv6= layers.Conv2D(256, kernel_size = 3, padding = 'same', activation = 'relu') (drop5)
    bn6 = layers.BatchNormalization() (conv6)
    relu6 = layers.ReLU() (bn6)
    drop6 = layers.Dropout(rate=0.26) (relu6)

    conv7 = layers.Conv2D(128, kernel_size = 3, padding = 'same', activation = 'relu') (drop6)
    bn7 = layers.BatchNormalization() (conv7)
    drop7 = layers.Dropout(rate=0.3) (bn7)
